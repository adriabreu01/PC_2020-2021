#include <iostream>
int mat [] = {100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119,
...
void swap(int* elemento1, int* elemento2) {
    int* auxiliar = elemento1;
    mat[*elemento2] = 400;
    std::cout << *elemento1 << std::endl; 
    std::cout << *elemento2 << std::endl;
}

void traspuesta(int pos, int filas) {
  for (int i{pos}; i < pos + filas; i++) { 
    for (int j{i}; j < i + (filas * 20); j = j + 20) {
      int* ptr =  mat + j;
      std::cout << *ptr << " ";
    }
    std::cout << std::endl;
...
int main() {
    ...
    do {
        ...
        std::cout << "(5) Traspuesta" << std::endl;
        ...
        if (option == 2) {
            std::cout << "Introduzca el elemento al que quiere acceder: " << std::endl;
            std::cin >> i;
            std::cin >> j;
            if (i < 0 || i > nfil) {
        ...
        if (option == 5) {
            ...
            if (nfil == ncol) {
                ...
                int filas = 0;
                std::cin >> filas;
                traspuesta(pos_base, filas);
            }
            else {
                std::cout << "Error: No se puede calcular la traspuesta en matrices no cuadradas." << std::endl;
            }
        }
